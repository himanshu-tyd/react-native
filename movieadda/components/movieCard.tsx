   ...
   <Link href={`/movie/${id}`}  asChild>
    <TouchableOpacity className='w-[30%] '>
        ...
        <View className='flex-row items-center justify-start  gap-x-1'>
                ...
                <Text className='text-white text-xs fond-bold uppercase  '>{Math.round(vote_average/2)}</Text>
        </View>
        <View className=' flex-row items-center justify-between'>
            <Text className='text-white/60 font-medium mt-1 text-xs'>{release_date?.split("-")[0]}</Text>
            <Text className=' text-xs font-medium text-white/60 uppercase '>
                Movie
            </Text>
        </View>
    </TouchableOpacity>
